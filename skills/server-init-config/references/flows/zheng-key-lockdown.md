...
Use the current target MCP config's provided `privateKey`, or the user-provided `privateKey` selected for this initialization. Derive `SSH_PUBLIC_KEY` with `ssh-keygen -y -f "$privateKey"` before running `ssh-prepare.sh`, and use the same provided `privateKey` for the temporary and final `zheng` MCP entries. Do not create or generate SSH keys.

Run `scripts/features/ssh-prepare.sh` first with that derived `SSH_PUBLIC_KEY`. It must keep bootstrap root/password and old SSH port available while opening port `17223`.
